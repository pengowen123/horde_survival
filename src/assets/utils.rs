    ...
    ($typ:ident, $asset_type:ident, $field:ident,
     ...
     $log_str:expr) => {
        ...
        impl<P: AsRef<Path>> $typ<P> {
            pub fn $name1(&mut self, name: &str, factory: &mut Factory) -> io::Result<()> {
            ...
            pub fn $name2(&mut self, name: &str, factory: &mut Factory) -> io::Result<&$asset_type> {
                match self.$field.get_mut(name) {
            ...
            pub fn $name3<S: Clone + Into<P>>(&mut self, names: &[(&'static str, S)]) {
